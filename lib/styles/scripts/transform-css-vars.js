...
function main() {
  ...
  if (args.length === 0 || args.includes('--help') || args.includes('-h')) {
    ...
    console.log('Usage:');
    console.log('  node transform-css-vars.js <input.css> [output.css] [--apply] [--light|--dark]');
    console.log('  ./transform-css-vars.js <input.css> [output.css] [--apply] [--light|--dark]\n');
    ...
    console.log('                AND generate lib/styles/theme/theme.webflow.css');
    console.log('  --light     - Use :root (light mode) values as fallbacks in theme.webflow.css');
    console.log('  --dark      - Use .dark (dark mode) values as fallbacks in theme.webflow.css [DEFAULT]\n');
    ...
    console.log('  # Apply theme as defaults + generate theme.webflow.css with dark mode fallbacks');
    console.log('  node transform-css-vars.js bubblegum.css --apply');
    console.log('  → Transforms to lib/styles/variables/default.css');
    console.log('  → Generates lib/styles/theme/theme.webflow.css (dark mode)\n');
    console.log('  # Apply theme with light mode fallbacks');
    console.log('  node transform-css-vars.js bubblegum.css --apply --light');
    console.log('  → Transforms to lib/styles/variables/default.css');
    console.log('  → Generates lib/styles/theme/theme.webflow.css (light mode)\n');
    ...
    console.log('  • When --apply is used, also generates lib/styles/theme/theme.webflow.css');
    console.log('  • Theme mode defaults to --dark if not specified');
    process.exit(0);
  }
  ...
  const themeMode = useLight ? 'light' : 'dark'; // Default to dark
  const filteredArgs = args.filter(arg => arg !== '--apply' && arg !== '--light' && arg !== '--dark');
  ...
  if (applyFlag) {
    ...
    // Generate theme.webflow.css
    console.log(`\n🔄 Generating theme.webflow.css with ${themeMode} mode fallbacks...\n`);

    const themeWebflowPath = path.resolve('lib/styles/theme/theme.webflow.css');
    const themeDir = path.dirname(themeWebflowPath);
    ...
    // Import and use theme generation functions
    const { parseRootVariables, parseDarkVariables, generateThemeBlock } = require('./generate-theme-webflow.js');

    // Parse variables based on mode
    let variables;
    if (themeMode === 'light') {
      variables = parseRootVariables(transformed);
      console.log(`📊 Found ${variables.size} variables in :root block\n`);
    } else {
      variables = parseDarkVariables(transformed);
      console.log(`📊 Found ${variables.size} variables in .dark block\n`);
    }

    // Generate and write theme content
    const themeContent = generateThemeBlock(variables);
    fs.writeFileSync(themeWebflowPath, themeContent, 'utf8');
    console.log(`✨ Generated theme file: ${path.relative(process.cwd(), themeWebflowPath)}`);
  }
  ...
  if (applyFlag) {
    console.log('\n📦 Summary:');
    console.log(`   ✓ Variables: lib/styles/variables/default.css`);
    console.log(`   ✓ Theme: lib/styles/theme/theme.webflow.css (${themeMode} mode)`);
  }
}
